...
def get_embedding_graph_from_node2vec_v1(df):
    
    G= nx.from_pandas_edgelist(df, "source", "target", ["orientation"])
    emb = 150
    g2v = Node2Vec(n_components=emb)
    g2v.fit(G)
    ix = list(G.nodes())
    X = np.zeros((len(ix),emb))
    cnt = 0
    for i in ix:
        X[cnt,:] = g2v.predict(i)
        cnt+=1
...
node_embeddings, node_names = get_embedding_graph_from_node2vec_v1(edge_information)
...
np.savetxt("data/Node2Vec_EMB_only.txt", data_emb)
