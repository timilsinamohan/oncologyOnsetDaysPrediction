...
def get_embedding_graph_from_rgcn(df):
    
    N = len(list(set(list(df["source"].values) + list(df["target"].values))))
    
    #N = len(list(set(df["source"].values)) + list(set(df["target"].values)))
    I = np.identity(N)
    #ix = list(set(df["source"].values)) + list(set(df["target"].values))
    ix = list(set(list(df["source"].values) + list(df["target"].values)))
    features = pd.DataFrame(data = I, index= ix)
    graph = sg.StellarGraph(
    edges= df, edge_type_column="orientation", nodes=features
        )
...
node_embeddings, node_names = get_embedding_graph_from_rgcn(edge_information)
...
np.savetxt("data/RGCN_EMB_only.txt", data_emb)
